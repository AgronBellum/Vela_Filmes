...
object UserPreferences {
    ...
    var currentProvider: Provider?
        ...
        set(value) {
            ...
            runCatching { ArtworkRepairScheduler.schedule(StreamFlixApp.instance, value) }
            // Notify all ViewModels that the provider has changed
            ProviderChangeNotifier.notifyProviderChanged()
        }

    fun getProviderCache(
        provider: Provider,
        key: String,
    ): String = providerCache.optJSONObject(provider.name)?.optString(key).orEmpty()

    fun setProviderCache(
        provider: Provider?,
        key: String,
        value: String,
    ) {
        ...
        val innerJson =
            providerCache.optJSONObject(providerName)
    ...
    var captionTextSize: Float
        get() =
            Key.CAPTION_TEXT_SIZE.getFloat()
    ...
    enum class PlayerResize(
        ...
        StretchVertical(
            R.string.player_aspect_ratio_stretch_vertical,
    ...
    var playerResize: PlayerResize
        get() =
            PlayerResize.entries.find {
                it.resizeMode == Key.PLAYER_RESIZE.getInt() &&
                    it.name == Key.PLAYER_RESIZE_NAME.getString()
            }
        ...
        get() =
            CaptionStyleCompat(
                Key.CAPTION_STYLE_FONT_COLOR.getInt()
                    ?: PlayerSettingsView.Settings.Subtitle.Style.DEFAULT
                        .foregroundColor,
    ...
    var streamingcommunityDomain: String
        ...
        set(value) {
            val oldDomain =
                if (::prefs.isInitialized) {
                    prefs.getString(Key.STREAMINGCOMMUNITY_DOMAIN.name, null)
                } else {
                    null
                }
            if (!::prefs.isInitialized) {
    ...
    var cuevanaDomain: String
        ...
        set(value) {
            val oldDomain =
                if (::prefs.isInitialized) {
                    prefs.getString(Key.CUEVANA_DOMAIN.name, null)
                } else {
                    null
                }
    ...
    var poseidonDomain: String
        ...
        set(value) {
            val oldDomain =
                if (::prefs.isInitialized) {
                    prefs.getString(Key.POSEIDON_DOMAIN.name, null)
                } else {
                    null
                }
            if (!::prefs.isInitialized) return
